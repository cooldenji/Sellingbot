...
bot.on('callback_query', async (query) => {
  ...
  try {
    if (data === 'adm_menu') {
      ...
      // ✅ Point 5: Text ID change
      getState(userId).waitingUpiTextId = true;
      await bot.sendMessage(userId, 'Enter UPI Text ID to display to users (e.g. yourname@bank):');
...
async function showPendingDeposits(userId, messageId) {
  ...
  for (const dep of deposits) {
    ...
    keyboard.push([
      { text: `👤 View Screenshot — ${dep.full_name}`, callback_data: `adm_view_ss_${dep.ref_id}` },
...
async function showProductsMenu(userId, messageId) {
  ...
  if (!products.length) {
    ...
    for (const p of products) {
      ...
      keyboard.push([
        { text: `✏️ Edit ${p.name}`, callback_data: `adm_edit_product_${p.id}` },
...
// ─── Sold Products ────────────────────────────────────────────────────────────
// ✅ Point 2: Admin sold products dekh sake
...
// ─── Delete Product ───────────────────────────────────────────────────────────
// ✅ Point 2: Product sirf admin manually delete kare, sell hone par nahi
...
// ─── UPI Settings ─────────────────────────────────────────────────────────────
// ✅ Point 5: UPI text ID + QR both supported
async function showUpiSettings(userId, messageId) {
  ...
  const text =
    ...
    `• Primary UPI ID — used for internal reference\n` +
    `• Display Text ID — shown to users on payment screen (with or without QR)`;
  ...
  try {
    await bot.editMessageText(
      ...
      `Reward per referral: *Rs ${reward}*`,
      {
        ...
        reply_markup: {
          inline_keyboard: [
            [{ text: enabled ? '❌ Disable Referral' : '✅ Enable Referral', callback_data: 'adm_toggle_referral' }],
...
bot.on('message', async (msg) => {
  ...
  if (state.waitingAddStock) {
    ...
    clearState(userId);
    await bot.sendMessage(userId,
      `✅ Added *${addCount}* stock to product!`,
  ...
  if (state.waitingNewPassword) {
    ...
    clearState(userId);
    await bot.sendMessage(userId,
      `✅ Password updated! New password: \`${text}\`\n\n⚠️ Note: This resets on server restart. Update your env variable too.`,
    ...
    for (const u of users) {
      try {
        await fetch(`https://api.telegram.org/bot${config.USER_BOT_TOKEN}/sendMessage`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ chat_id: u.user_id, text, parse_mode: 'Markdown' })
        });
        success++;
  ...
  // ─── Unknown message ──────────────────────────────────────────────────────
  // ✅ Point 4: Unknown command in admin
  await bot.sendMessage(userId,
    `ℹ️ Use /start to access the admin panel.`
  );
});
