...
async function deliverPurchase(userId, purchaseId) {
  ...
  if (!purchase) {
    await bot.sendMessage(userId, '❌ Purchase not found.');
    return;
  }

  // User ko product deliver karo
  const userMsg =
    `✅ *Your Order is Ready!*\n\n` +
  ...
  try {
    const userApiUrl = `https://api.telegram.org/bot${config.USER_BOT_TOKEN}/sendMessage`;
  ...
  await bot.sendMessage(userId,
    ...
    {
      parse_mode: 'Markdown',
      reply_markup: { inline_keyboard: [[{ text: '« Back to Menu', callback_data: 'adm_menu' }]] }
    }
  );
}
