...
bot.onText(/\/start(?:\s+(.+))?/, async (msg, match) => {
  ...
  // ✅ Point 1: Agar terms already accepted hai to seedha menu
...
async function processPurchase(userId, messageId, productId) {
  ...
  // ✅ Point 2: Stock ghata, total_sold badha — product tab tak active jab tak stock 0 na ho
  await database.run(
...
async function showQrAndPaymentDetails(userId, amount, upiApp) {
  ...
  // ✅ Point 5: UPI ID text bhi show karo (pic ke saath ya bina pic ke bhi)
  if (upiTextId) {
  ...
  // ✅ Point 3: Screenshot system — QR pic bhejo pehle, phir user screenshot bheje
  if (qrFileId) {
...
// ─── Message Handler ─────────────────────────────────────────────────────────
// ✅ Point 4: Faltu messages handle karo
bot.on('message', async (msg) => {
  if (msg.text && msg.text.startsWith('/')) return; // commands alag handle hote hain
  if (msg.photo) {
  ...
  // ✅ Point 4b: / ke baad word bhi work kare
  const commandMap = {
    'menu': 'main_menu',
    'start': null,
    'buy': 'buy_product',
  ...
  if (commandMap.hasOwnProperty(lower)) {
    ...
    const fakeQuery = {
      from: msg.from,
      data: commandMap[lower],
      message: { message_id: msg.message_id },
      id: 'fake'
    };
    // redirect to callback handler logic
  ...
  // ✅ Point 4: Unknown message — professional response
  const dbUser = await db.getUser(userId);
...
// ✅ Point 3: User screenshot bheje to admin bot pe forward ho with pic
async function handlePhotoMessage(msg) {
  ...
  // Save deposit to DB with screenshot
  await database.run(
  ...
  // ✅ Point 3: Admin bot pe forward karo screenshot ke saath
  try {
    const adminCaption =
      `📥 *New Deposit Request*\n\n` +
...
async function handleTextCommand(userId, callbackData) {
  ...
  else if (callbackData === 'buy_product') await showProducts(userId, null).catch(() => {});
  else if (callbackData === 'profile') await showProfile(userId, null).catch(() => {});
  else if (callbackData === 'deposit') await showDepositMenu(userId, null).catch(() => {});
  else if (callbackData === 'refer_earn') await showReferral(userId, null).catch(() => {});
  else if (callbackData === 'support') await showSupport(userId, null).catch(() => {});
}
