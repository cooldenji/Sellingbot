...
// ─── Process Purchase ─────────────────────────────────────────────────────────
// ✅ POINT 2: Admin ko notification, user ko waiting message
async function processPurchase(userId, messageId, productId) {
  ...
  // ✅ Admin ko purchase notification bhejo via Admin Bot
  try {
    ...
    const adminCaption =
      ...
      `💵 *User Balance After:* Rs ${(dbUser.balance_inr - product.price_inr).toFixed(0)}\n\n` +
    ...
    const adminApiUrl = `https://api.telegram.org/bot${config.ADMIN_BOT_TOKEN}/sendMessage`;
    ...
    console.error('Failed to notify admin of purchase:', err.message);
  }
}
...
// ✅ POINT 1: QR ya Text ID — jo set ho wahi dikhao, permanently stored
async function showQrAndPaymentDetails(userId, amount, upiApp) {
  ...
  // DB se QR aur Text ID dono fetch karo
  ...
  // Display UPI ID — text id pehle, phir upi_id
  ...
  // ✅ Agar QR hai to photo ke saath bhejo, warna sirf text
...
// ✅ POINT 1: Screenshot Admin Bot pe jaaye with photo + approve buttons
async function handlePhotoMessage(msg) {
  ...
  await bot.sendMessage(userId,
    ...
    `Approval within *15–30 minutes*.\n\n` +
    `Keep your Ref ID safe for any queries.`,
    { parse_mode: 'Markdown' }
  );

  // ✅ POINT 1 FIX: Admin Bot pe photo bhejo — fetch with ADMIN_BOT_TOKEN
  try {
    const adminCaption =
      `📥 *New Deposit Request*\n\n` +
      `👤 *User:* ${dbUser.full_name}\n` +
    ...
    // ✅ ADMIN BOT TOKEN se photo send ho raha hai
    ...
    if (!result.ok) {
      console.error('Admin notify failed:', JSON.stringify(result));
    }
